...
namespace RealEstateAnalysis
{
    public class ApartmentData
    {
        ...
        public int Rooms { get; set; }  // Убрал internal для более гибкого доступа
    }
    ...
    class Program
    {
        ...
        // [Ваш существующий метод ParseAvito без изменений]
        ...
        //        var hist = plt1.AddHistogram(prices, 20);
        //        hist.FillColor = System.Drawing.Color.SteelBlue;
        //        plt1.SaveFig("price_distribution.png");

        //        // Диаграмма рассеяния: цена vs площадь
        //        var plt2 = new ScottPlot.Plot(800, 600);
        //        plt2.Title("Зависимость цены от площади");
        //        plt2.XLabel("Площадь (м²)");
        //        plt2.YLabel("Цена (млн руб)");
        //        var scatter = plt2.AddScatter(
        //            apartments.Select(a => (double)a.Square).ToArray(),
        //            prices,
        //            markerSize: 5,
        //            color: System.Drawing.Color.Red
        //        );
        //        plt2.SaveFig("price_vs_area.png");
        //    }
        //    catch (Exception ex)
        //    {
        ...
        static void BuildRegressionModel(List<ApartmentData> apartments)
        {
            try
            {
                ...
                // Подготовка данных
                var data = mlContext.Data.LoadFromEnumerable(apartments.Select(a => new
                {
                    Price = a.Price,
                    Square = a.Square,
                    Rooms = (float)a.Rooms,
                ...
                var pipeline = mlContext.Transforms
                    .Concatenate("Features", nameof(ApartmentData.Square), nameof(ApartmentData.Rooms), nameof(ApartmentData.Floor))
                ...
                // Пример прогноза
                var predictionEngine = mlContext.Model.CreatePredictionEngine<ApartmentData, PricePrediction>(model);
                var sample = apartments[Random.Shared.Next(apartments.Count)];
                var prediction = predictionEngine.Predict(sample);

                Console.WriteLine("\nПример прогноза:");
                Console.WriteLine($"• Реальная цена: {sample.Price:N0} руб.");
                Console.WriteLine($"• Прогнозируемая: {prediction.PredictedPrice:N0} руб.");
                Console.WriteLine($"• Параметры: {sample.Square} м², {sample.Rooms}к, этаж {sample.Floor}/{sample.TotalFloors}");
            }
            catch (Exception ex)
            {
